...
export default function AdminMonitoringPage() {
    ...
    return (
        <div className="space-y-8">
            <div className="rounded-2xl border border-[#466A68]/20 bg-white p-6 lg:p-8">
                <div className="flex items-center gap-3">
                    <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#466A68]/20 text-[#7ca3a0]">
                        <Activity className="h-5 w-5" />
                    </div>
                    <div>
                        <h1 className="text-xl font-bold text-[#0F0A09]">Monitoring Operasional</h1>
                        <p className="text-xs text-[#8C7A6B]/50">Ringkasan metrik 24 jam terakhir untuk admin API</p>
                    </div>
                </div>
                <p className="mt-4 text-xs text-[#8C7A6B]/45">Window: {windowLabel}</p>
            </div>
                ...
                {metricCards.map((card) => (
                    <div key={card.label} className={`rounded-xl border p-4 ${card.cardClass}`}>
                        <div className="flex items-start justify-between">
                            <div>
                                <p className="text-[11px] uppercase tracking-wide text-[#8C7A6B]/50">{card.label}</p>
                                <p className="mt-2 text-3xl font-bold text-[#0F0A09] tabular-nums">{card.value}</p>
                                <p className="mt-1 text-[11px] text-[#8C7A6B]/45">{card.hint}</p>
                            </div>
            ...
            <section className="rounded-xl border border-[#D4BCAA]/20 bg-white overflow-hidden">
                <div className="border-b border-[#D4BCAA]/20 px-5 py-4">
                    <h2 className="text-sm font-semibold text-[#0F0A09]">Top Endpoint (24 Jam)</h2>
                    <p className="mt-1 text-xs text-[#8C7A6B]/40">Berdasarkan field action dari event request admin</p>
                </div>

                {snapshot.topEndpoints.length === 0 ? (
                    <div className="px-5 py-8 text-center text-sm text-[#8C7A6B]/35">Belum ada event request yang tercatat.</div>
                ) : (
                        ...
                        {snapshot.topEndpoints.map((endpoint, index) => (
                            <div key={endpoint.action} className="flex items-center justify-between px-5 py-3">
                                <div className="min-w-0">
                                    <p className="text-sm text-[#0F0A09] truncate">#{index + 1} {endpoint.action}</p>
                                </div>
